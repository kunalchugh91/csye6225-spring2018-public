...
RC=$(aws cloudformation create-stack --stack-name $1-ci-cd --capabilities "CAPABILITY_NAMED_IAM" --template-body file://./csye6225-cf-ci-cd.json --parameters ParameterKey=S3ARN,ParameterValue=arn:aws:s3:::$WEBAPP_DOMAIN ParameterKey=CDAPPNAME,ParameterValue=CodeDeployApplication ParameterKey=CDOMAIN,ParameterValue=$CD_DOMAIN)
